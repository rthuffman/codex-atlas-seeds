...
def sync_usg_packs(*, athena_root: Path | None = None, repo_root: Path | None = None) -> list[Path]:
    ...
    mappings = [
        (fixture_dir / "us_administration_skeleton_catalog.json", root / "packs" / "usg_administration_skeleton" / "catalog.json"),
        (fixture_dir / "us_statutory_cabinet_departments_timeline.json", root / "packs" / "usg_statutory_cabinet_timeline" / "timeline.json"),
    ]
    written: list[Path] = []
